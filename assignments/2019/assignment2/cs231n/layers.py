...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    X = x.reshape(N, np.prod(x[0].shape))
    out = X.dot(w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #gamma 是最终的var, beta 是u; 这两个参数是为了给bn灵活性，万一这个层次需要的是方差和期望略有区别的gaussian呢？
    #那么加了这两参数，就可以给出这个灵活性，让neuron去学习合适的参数
    mode = bn_param['mode']
    ...
    if mode == 'train':
        ...
        cache={'x':x,'mean':sample_mean,'std':std,'gamma':gamma,'z':z,'var':sample_var,'axis':layernorm}
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dout = dout.transpose(0, 3, 2, 1).reshape(N*W*H, C) #想要reshape，数据的分布得对应的上
